import React from 'react';
//import SearchBox from '../SearchBox/SearchBox';
import navClass from './Nav.css';

const Nav = (props) => {

    const SearchBoxStyle = {
        width:'100%',
        height:'20px',
        padding:'5px',
        border:'0px',
        margin:'0 10px 0 0'

    }

    const cartIconStyle = {
        maxWidth:'30px'
    }
 
    return(
        <nav className = {navClass.header}>
            <ul className = {navClass.mainNav}>
                <li id = {navClass.bars}><i className = "fa fa-bars fa-2x"></i></li> 
                <li className = {navClass.lapLi} onClick = { () => props.click(0)}>首頁</li>
                <li className = {navClass.lapLi} onClick = { () => props.click(1)}>牌組編輯器</li>
                <li className = {navClass.lapLi} onClick = { () => props.click(0)}>關於本站</li>
                <form className = {navClass.search}>
                    <input type = {navClass.text} name='productName' id = {navClass.productName} style = {SearchBoxStyle}  />
                    <input className = {navClass.addSearchIcon} type='button' data-action="submit" onClick = {() => props.click(3)}/>
                </form>
                <div className = {navClass.push} >
                    <li className = {navClass.lapLi} onClick = { () => props.click(0)}>登入</li>
                    <li onClick = { () => props.click(2)}><img style = {cartIconStyle} alt = "購物車" src="data:image/svg+xml;base64," /></li>
                </div>
            </ul>
        </nav>
    )
}

             

export default Nav;

